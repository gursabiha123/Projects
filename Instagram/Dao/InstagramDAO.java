...
//create profile - file entry by entry
	/*
	public int createProfileDAO(Instagramuser iu) {
		int i=0;
		try {
...
*/ 
	...
	//create profile - file - by copying objects
	public int createProfileDAO(Instagramuser iu){
		int i=0;try {
	  ...
	  ObjectOutputStream oo=new ObjectOutputStream(out);
		...
		/*
		 * Instagramuser ee=new Instagramuser(); ee.setName(iu.getName());
		 * ee.setAddress(iu.getAddress()); ee.setPassword(iu.getPassword());
		 * ee.setEmail(iu.getEmail());
		 */
	  oo.writeObject(iu);
	...
	File f2=new File("d:/hsbc_file/two.txt");
	
	FileInputStream in=new FileInputStream(f2);
	
	ObjectInputStream ooo=new ObjectInputStream(in);
	
	//put while loop until EOF to read all objects else only 1 will be read		
	Instagramuser e=(Instagramuser)ooo.readObject();//(Instagramuser is typecasting)
	
	System.out.println(e.getName()+"  "+e.getPassword()+" "+e.getEmail()+" "+e.getAddress());
